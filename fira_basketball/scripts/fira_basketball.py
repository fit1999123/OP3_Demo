...
func2 = detect2Color
args1 = ((np.array([13, 145, 150]), np.array([25, 255, 255])),)# hsv value for ping pong
#0,130,0
...
# Iinitialize Node
rospy.init_node("fira_basketball_2021")
...
def init():
    ...
    #rospy.sleep(1.0)
    #for i in range(4):
    #    l_grip_pub.publish(0.0)
    #    rospy.sleep(0.1)
    #rospy.sleep(1.0)
    #for i in range(4):
    #    r_grip_pub.publish(0.0)
    #    rospy.sleep(0.1)
    #rospy.sleep(1.0)
...
while not rospy.is_shutdown():
    ...
    if currState == States.INIT:
        ...
        if tick_count > tickrate // 3:
            ...
            robot.walkStart()
            robot.walkVelocities(x=-7.5, th=0)
            rospy.sleep(3)
            robot.walkStop()
            direction = False
            finished_cycle = False
