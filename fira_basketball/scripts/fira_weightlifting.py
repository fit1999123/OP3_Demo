...
while not rospy.is_shutdown():
    ...
    slope_adjust = True
    slope_limit = 0.05 #original 0.02
    delta_head = 0
    delta_lr = 0
    bar_slope = 0 
    ...
    if currState == States.INIT:
        ...
        robot.walking_params[1].y_move_amplitude = -0.002
        robot.walking_params[1].balance_enable = True
        # robot.walking_params[1].angle_move_amplitude = -2.5
        ...
        if slope_adjust==True:
            ...
            while bar_slope <= -slope_limit or bar_slope > slope_limit and bar_slope!=0:
                ...
                if bar_slope <= -slope_limit:  #original 0.02
                    ...
                    robot.onlineWalkCommand(direction="turn_left", start_leg="right", step_num=2,front_length=-0.1, step_angle=5*np.abs(bar_slope),step_time=0.5)
                    rospy.sleep(5)
                    ...
                    robot.onlineWalkCommand(direction="turn_right", start_leg="right", step_num=2,front_length=-0.1, step_angle=5*np.abs(bar_slope),step_time=0.5)
                    rospy.sleep(5)
        ...
        robot.playMotion(75, wait_for_end=True)
        rospy.sleep(1.0)
        ...
        robot.walking_params[2].hip_pitch_offset = 0.1 #nice garbage
        ...
        # robot.setGrippersPos(left=50)
        # robot.setGrippersPos(right=50)
        ...
        robot.walking_params.append(robot.loadWalkingParams('pickup_param.yaml'))
        robot.walking_params[3].hip_pitch_offset = -2 * DEGREE2RADIAN #1.5
        robot.walking_params[3].x_move_amplitude = 0
        robot.walking_params[3].balance_enable = True
        robot.walking_params[3].angle_move_amplitude = 10 * DEGREE2RADIAN
